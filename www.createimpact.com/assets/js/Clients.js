// Moving sphere with different skills

const canvas = document.getElementById('canvas');

const texts = [
  'Capital Tv', 'Spintex', 'Bashir Fast Food', 'COL', 'StarLink',
  'Stellar Marine', 'TLC', 'IRADA Logistics', 'Speciality Food',
];
...
function wordSphere(canvas, texts, counts, options) {
  ...
  function render() {
    ...
    for (const text of texts) {
      ...
      const alpha = 0.8 + 0.4 * (x/radius);
      const size = fontSize + 2 + 5*(x/radius);
      ctx.fillStyle = `rgba(256,182,0,${alpha})`;
